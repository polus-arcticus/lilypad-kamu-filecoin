...
contract DealClient712 {
  bytes32 DOMAIN_SEPARATOR;
  bytes EXTRA_PARAMS_V1_TYPE= "ExtraParamsV1(string location_ref,uint64 car_size,bool skip_ipni_announce,bool remove_unsealed_copy)";
  bytes DEAL_REQUEST_TYPE = abi.encodePacked("DealRequest(bytes piece_cid,uint64 piece_size,bool verified_deal,string label,int64 start_epoch,int64 end_epoch,uint256 storage_price_per_epoch,uint256 provider_collateral,uint256 client_collateral,uint64 extra_params_version, EXTRA_PARAMS_V1_TYPE extra_params)", EXTRA_PARAMS_V1_TYPE);
  ...
  // Deal Proposals now made of EIP-712 message formed offchain
